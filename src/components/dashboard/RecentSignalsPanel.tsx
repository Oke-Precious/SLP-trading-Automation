 ...
 * @description List of recently executed algorithmic setups fetched from the real backend API.
 */
    ...
    <div className="bg-card border border-border-custom rounded-xl p-5 hover:border-[#3A455E] transition-all duration-300">
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center space-x-2">
          ...
          <h3 className="text-sm font-bold uppercase tracking-wider text-white font-display">Algorithmic Signals</h3>
        </div>
        <div className="flex items-center space-x-2">
          <button
            onClick={() => refetch()}
      ...
      <div className="space-y-2.5 max-h-[290px] overflow-y-auto pr-1">
        {isLoading ? (
          ...
          signals.map((sig) => (
            <div 
              key={sig.id}
              className="flex items-center justify-between p-3 rounded-lg bg-surface border border-[#2D313E]/30 text-xs hover:border-[#3A455E] transition-colors"
            >
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <div className="flex items-center space-x-1.5">
                    <span className="font-bold text-white uppercase">{sig.pair}</span>
